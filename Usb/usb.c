...
void usb_init()
{
    ...
    usb_write_reg(INTRIN1E, 0x3f);//允许全部端点IN中断
    usb_write_reg(INTROUT1E, 0x3f);//允许全部端点OUT中断
    usb_write_reg(INTRUSBE, 0x07);//允许部分电源中断
    usb_write_reg(POWER, 0x01);//不忽略挂起信号
...
BYTE usb_read_fifo(BYTE fifo, BYTE *pdat)
{
    ...
    while (cnt--) *pdat++ = usb_read_reg(fifo);
...
void usb_isr() interrupt 25 //USB中断处理函数
{
    BYTE intrusb;
    BYTE intrin;
    BYTE introut;
...
void usb_out_ep1()
{
    ...
    if (csr & OUTOPRDY){
		if(usbMode == 0) EP1_Out_Led();	//用于键盘LED控制
		else scsi_process();			//U盘模式相关处理
    }
}
#endif
...
void usb_out_ep2()
{
    ...
    if (csr & OUTOPRDY){
		if(usbMode == 0) EP2_Out_Cmd();//控制与配置命令
        //usb_bulk_intr_out(Ep2OutBuffer, 2);
		//usb_bulk_intr_in(UsbBuffer, usb_bulk_intr_out(UsbBuffer, 2), 2);//环回测试
    }
}
#endif
