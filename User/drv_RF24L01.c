...
void NRF24L01_Nop( void )
{
    ...
    drv_spi_read_write_byte( NOP_CMD );		//空操作命令
...
*			@times:最大重复次数,0为无穷
  ...
  * @retval:无
  */ 
uint8_t NRF24L01_check(uint8_t times)
{
	uint8_t i, j;
	uint8_t checkBuf[5] = {0xA5, 0xA5, 0xA5, 0xA5, 0xA5};
	 
	for(j = 0; j < times || !times; j++)//未达到最大重复次数或为无穷次
	{
		NRF24L01_Write_Buf(TX_ADDR, checkBuf, 5);//写入5个字节的地址
		memset(checkBuf, 0, 5);
		NRF24L01_Read_Buf(TX_ADDR, checkBuf, 5);//读出写入的地址
		for(i = 0; i < 5; i++){
			if(checkBuf[i] != 0xA5) break;//任何一个不对就跳出比对
		}
		
		if(i == 5) return 1;//全部比对成功则返回
		else{
			//printf("NRF not connect\n");
			//drv_uart_tx_bytes( (uint8_t *)g_ErrorString, 26 );//未接入NRF24L01
		}
		//drv_delay_500Ms( 4 );
		Delay_ms(100);
	}
...
uint8_t NRF24L01_TxPacket( uint8_t *txbuf, uint8_t Length )
{
	...
	while( 0 != RF24L01_GET_IRQ_STATUS( ))
	{
		//Delay_ms( 1 );
		Delay_us(200);
		if( 2500 == l_MsTimes++ )						//500ms还没有发送成功，重新初始化设备
		{
			NRF24L01_Gpio_Init( );
			RF24L01_Init( );
...
{
//	GPIO_Init(RF24L01_CE_GPIO_PORT, RF24L01_CE_GPIO_PIN, MODE3);
//	GPIO_Init(RF24L01_CS_GPIO_PORT, RF24L01_CS_GPIO_PIN, MODE3);
//	GPIO_Init(RF24L01_IRQ_GPIO_PORT, RF24L01_IRQ_GPIO_PIN, MODE3/*MODE0*/);//0为高阻输入,1为推挽,2为开漏,3为准双向

	RF24L01_SET_CE_HIGH();
	P3_MODE_IO_PU(GPIO_Pin_4);//CE准双向
	RF24L01_SET_CE_LOW( );
	RF24L01_SET_CS_HIGH( );
...
void RF24L01_Init( void )
{
    ...
    NRF24L01_Write_Reg( FEATRUE, 0x04 );//(源代码为0x07)
    ...
    NRF24L01_Write_Reg( RF_CH, RF_INIT_CH );             	//初始化通道(源代码为60)
    NRF24L01_Write_Reg( RF_SETUP, 0x26|0x01 );//配置(源代码为0x26)
...
uint8_t NRF24L01_TxStart(uint8_t *txbuf, uint8_t Length)
{
	RF24L01_SET_CS_LOW( );		//片选
	drv_spi_read_write_byte( FLUSH_TX );
	RF24L01_SET_CS_HIGH( );
...
uint8_t NRF24L01_TxCheck(void)
{
	uint8_t l_Status = 0;
	//uint16_t l_MsTimes = 0;
	static uint8_t TxCount = 0;//发送等待计数变量
	if(0 != RF24L01_GET_IRQ_STATUS( )){//若还未发送成功
		TxCount++;
		if(TxCount >= 20){//100ms还未发送成功
			TxCount = 0;//计数清零
			return 0xFE;//返回超时标志
		}
		return 0xFD;//返回正在发送标志
	}
//	while( 0 != RF24L01_GET_IRQ_STATUS( ))
//	{
//		//Delay_ms( 1 );
...
/**
  * @brief :NRF24L01接收启动(自编)
  * @param :
  * @note  :
  * @retval:接收的数据个数
  */
uint8_t NRF24L01_RxStart(void)
{
	RF24L01_SET_CS_LOW( );//片选
	drv_spi_read_write_byte( FLUSH_RX );
	RF24L01_SET_CS_HIGH( );
...
/**
  * @brief :NRF24L01接收数据(自编)
  * @param :
  *			@rxbuf:接收数据存放地址
  * @note  :
  * @retval:接收的数据个数
  */
uint8_t NRF24L01_RxPkg(uint8_t *rxbuf)
{
	uint8_t l_Status = 0, l_RxLength = 0;
	static uint8_t RxCount = 0;//接收等待计数变量
	
	if( 0 != RF24L01_GET_IRQ_STATUS( ))
	{
		RxCount++;
		if(RxCount >= 100){//500ms未收到数据
			RxCount = 0;//计数清零
			return 0xFF;//返回断连标志
		}
		return 0;//直接返回
 ...
 /**
  * @brief :RF24L01模块初始化与配置
  * @param :无
  * @note  :配置为接收模式并启动接收
  * @retval:无
  */
void NRF24L01_Config(uint8_t ifTX)
{
	NRF24L01_Gpio_Init( );//RF24L01控制引脚初始化
	G24_Chip_State |= 0x40;
	if(NRF24L01_check(5)){//检测到nRF24L01
		G24_Chip_State |= 0x80;
		RF24L01_Init( );//RF24L01初始化
		if(!ifTX){//若不为发送模式
			RF24L01_Set_Mode( MODE_RX );//RF24L01设为接收模式
			NRF24L01_RxStart();//启动接收
		}
	}
}
